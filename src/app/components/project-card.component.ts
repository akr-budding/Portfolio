    ...
    <article class="panel section-card group flex h-full flex-col overflow-hidden">
      ...
      >
        ...
        <div class="absolute left-5 top-5">
          <span
            class="rounded-full border border-white/10 bg-[var(--color-panel)]/80 px-3 py-1 text-xs font-semibold uppercase tracking-[0.28em] text-[var(--color-accent)]"
          >
            Project
          </span>
        </div>
      ...
      <div class="mt-6 flex flex-1 flex-col">
        <div class="space-y-3">
          <p class="text-sm font-medium uppercase tracking-[0.28em] text-[var(--color-accent-2)]">
            {{ project.subtitle }}
          </p>
        ...
        <div class="mt-6 flex flex-wrap gap-2">
          @for (item of project.stack; track item) {
            <span
              class="rounded-full border border-white/10 px-3 py-1 text-xs text-[var(--color-muted)]"
              >{{ item }}</span
            >
          }
        </div>
